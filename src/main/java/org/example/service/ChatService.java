...
public class ChatService {
     ...
     * @return 完整的系统提示词
     */
    public String buildSystemPrompt(List<Map<String, String>> history, String rollingSummary) {
        ...
        if (rollingSummary != null && !rollingSummary.isBlank()) {
            systemPromptBuilder.append("--- 历史摘要（较早对话的压缩结果） ---\n");
            systemPromptBuilder.append(rollingSummary).append("\n");
            systemPromptBuilder.append("--- 历史摘要结束 ---\n\n");
        }
        
        // 添加历史消息
        if (!history.isEmpty()) {
            systemPromptBuilder.append("--- 最近对话（保留原始细节） ---\n");
        ...
        systemPromptBuilder.append("请综合历史摘要与最近对话，优先保持上下文连续性，再回答用户的新问题。");
        
        return systemPromptBuilder.toString();
    }

    public void logAvailableTools(ToolAccessContext toolAccessContext) {
        toolAssemblyService.logAvailableTools(toolAccessContext);
    }
     ...
     */
    public ReactAgent createReactAgent(DashScopeChatModel chatModel, String systemPrompt, ToolAccessContext toolAccessContext) {
